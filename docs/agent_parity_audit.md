...
| Startup | `cargo run` default | Partial | `default-run = "deepseek-code"` in `Cargo.toml`. | Bare run enters CLI welcome/chat, not Ratatui TUI. Product default is not yet decided. | Decide whether default should be TUI, then add smoke tests for bare `cargo run` and `deepseek-code tui`. |
| Commands | Built-in slash commands | Partial | Registry in `src/commands/mod.rs`; TUI prefix hints and Tab completion in `src/tui/app.rs`; local `/tasks`; forward tests for `/run`, `/ask`, `/plan`. | No custom command file discovery; `# memory` is only a hint; `/run` etc. are forwarded rather than first-class command flows. | Add `.deepseek-code/commands` and user command loading; add a full command palette and registry snapshot tests. |
...
| Skills | Skills | Missing | Welcome skills are hard-coded tool display in `src/tui/welcome.rs`; no `SKILL.md` loader found. | No skills directory discovery, metadata, preload, or prompt injection. | Add `.deepseek-code/skills/<name>/SKILL.md` loader and prompt injection strategy. |
