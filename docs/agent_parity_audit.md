...
| Startup | `cargo run` default | Partial | `default-run = "octo"` in `Cargo.toml`; `octocode` remains as a compatibility binary. | Bare run enters CLI welcome/chat, not Ratatui TUI. Product default is not yet decided. | Decide whether default should be TUI, then add smoke tests for bare `cargo run` and `octo tui`. |
